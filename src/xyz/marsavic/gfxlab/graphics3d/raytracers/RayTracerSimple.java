...
public class RayTracerSimple extends RayTracer {
	...
	protected Color sample(Ray ray, int depthRemaining) {
		...
		if (material.refractive().notZero()) {
			double k = material.refractiveIndex();
			double cosA = n_.dot(i) / lI;
			Vec3 f = r;
			int angle = 1;
			...
			double cosBSqr = 1 - (1 - cosA * cosA) / (k * k);

			if (cosBSqr > 0) {
				double cosB = angle * Math.sqrt(cosBSqr);
				f = n_.mul(cosA / k - cosB).sub(i.div(k * lI));
			}

			Color lightRefracted = sample(Ray.pd(p, f), depthRemaining - 1).mul(material.refractive());
			result = result.add(lightRefracted);
		}
